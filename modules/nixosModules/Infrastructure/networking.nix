...
lib.mkIf enableInfrastructure {
  ...
  systemd.services."iptables-router" = {
    ...
    script = ''
      ...
      $IPTABLES --table filter --append INPUT --destination 224.0.0.0/24 --jump ACCEPT        # IPv4 多播
      $IPTABLES --table filter --append INPUT --protocol icmp --icmp-type echo-request --jump ACCEPT # 允许 ping
      $IPTABLES --table filter --append INPUT --protocol icmp --icmp-type destination-unreachable --jump ACCEPT # 路由/端口不可达
      $IPTABLES --table filter --append INPUT --protocol icmp --icmp-type time-exceeded --jump ACCEPT # TTL 超时
